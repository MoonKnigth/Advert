...
export default function MediaPage() {
  ...
  // Toast
  const [toast, setToast] = useState<{ open: boolean; msg: string; sev: 'success' | 'error' | 'warning' | 'info' }>({
    open: false,
    msg: '',
    sev: 'success'
  })
  ...
  // เดิม: const handleDelete = async () => {
  const handleDelete = async (idsOverride?: number[]) => {
    setDeleting(true)
    ...
    try {
      ...
      if (res.ok) {
        ...
        setToast({ open: true, msg: 'ลบไม่สำเร็จ', sev: 'error' })
      }
    } catch (err) {
      console.error('Delete error:', err)
      setError('Error occurred while deleting')
      setToast({ open: true, msg: 'เกิดข้อผิดพลาดระหว่างลบ', sev: 'error' })
    } finally {
      setDeleting(false)
    }
  }
  ...
  // item สำหรับโชว์ชื่อเวลาลบเดี่ยว
  const confirmItem = useMemo(
    () => (confirmIds.length === 1 ? media.find(m => m.id === confirmIds[0]) : null),
    [confirmIds, media]
  )

  // helpers
  const openConfirm = (ids: number[]) => {
  ...
  const handleEditSave = async (title: string, description: string) => {
    ...
    if (!NAME_ALLOWED_RE.test(name)) {
      setToast({
        open: true,
        msg: 'ชื่ออนุญาตเฉพาะตัวอักษรทุกภาษา/ตัวเลข และสัญลักษณ์: เว้นวรรค, _ , -',
        sev: 'warning'
      })
  ...
  return (
    <>
                            ...
                            >
                              ...
                              <MenuItem
                                ...
                                onClick={e => {
                                  e.stopPropagation()
                                  handleMenuClose()
                                  openConfirm([item.id]) // ← เปิดยืนยันลบเดี่ยว
                                }}
                              >
                                <ListItemIcon>
      ...
      <Dialog open={confirmOpen} onClose={closeConfirm} maxWidth='xs' fullWidth>
        ...
        <DialogContent>
          <Typography sx={{ mb: 1.5 }}>ต้องการลบโฆษณาจริง ๆ ใช่ไหม?</Typography>

          {confirmIds.length === 1 && confirmItem ? (
            <Alert severity='warning' variant='outlined'>
              จะลบรายการ: <strong>{confirmItem.title || `#${confirmItem.id}`}</strong>
            </Alert>
          ) : (
            <Alert severity='warning' variant='outlined'>
              จะลบทั้งหมด <strong>{confirmIds.length}</strong> รายการ
            </Alert>
          )}
        </DialogContent>
        <DialogActions>
          <Button onClick={closeConfirm} variant='tonal' color='secondary' disabled={deleting}>
            ยกเลิก
          </Button>
          <Button
            onClick={async () => {
              // ลบเดี่ยว/หลายรายการตามที่เลือก
              if (confirmIds.length === 1) {
                await handleDeleteSingle(confirmIds[0])
              } else {
                await handleDelete(confirmIds) // ใช้ idsOverride
              }
